...
@Getter
@Setter
@Component("scheduleJava8View")
@Scope("session")
public class ScheduleJava8View implements Serializable {

	/*
	 * @Inject private ExtenderService extenderService;
	 */
   ...
   // private Map<String, ExtenderExample> extenderExamples;
	...
	/*
	 * public ExtenderService getScheduleExtenderService() { return extenderService;
	 * }
	 * 
	 ...
	 * ExtenderExample example = getExtenderExample(); if
	 * (!"custom".equals(selectedExtenderExample) && example != null) { if
	 * (example.getDetails() != null && !example.getDetails().isEmpty()) {
	 * FacesMessage message = new FacesMessage(example.getName(),
	 * example.getDetails());
	 * FacesContext.getCurrentInstance().addMessage(event.getComponent().getClientId
	 * (), message); } this.extenderCode = example.getValue(); } }
	 */
	...
	/*
	 * public ExtenderExample getExtenderExample() { return
	 * extenderExamples.get(selectedExtenderExample); }
	 */


	/*
	 * public List<SelectItem> getExtenderExamples() { return
	 * extenderExamples.values().stream() //
	 * .sorted(Comparator.comparing(ExtenderExample::getName)) // .map(example ->
	 * new SelectItem(example.getKey(), example.getName())) //
