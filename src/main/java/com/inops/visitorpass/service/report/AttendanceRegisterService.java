...
@Service("attendanceRegister")
public class AttendanceRegisterService implements DataExtractionService {

	private IMuster musterService;

	public AttendanceRegisterService(IMuster musterService) {
		super();
	...
	public List<AttendanceRegister> dataExtraction(LocalDate from, LocalDate to, List<Employee> employeeIds) {
		...
		muster.get().stream().forEach(data -> {
			...
			if (attendanceRegistry.containsKey(data.getMusterId().getEmployeeId())) {
				...
				setAttendanceRegister(attendanceRegister, day, data.getAttendanceId());
			} else {
	...
	private void setNameAndDepartment(AttendanceRegister attendanceRegister, String employeeId, List<Employee> employeeIds)
	{
		Employee employee = employeeIds.stream().filter(emp->emp.getEmployeeId().equalsIgnoreCase(employeeId)).findAny().orElse(null);
	...
	private void setAttendanceRegister(AttendanceRegister attendanceRegister, int day, String attid) {
		switch (day) {
		case 1:
