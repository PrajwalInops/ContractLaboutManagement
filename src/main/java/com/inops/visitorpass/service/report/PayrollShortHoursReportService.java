...
@Service("payrollShortHoursService")
@RequiredArgsConstructor
public class PayrollShortHoursReportService implements DataExtractionService {
		...
		employeeIds.stream().forEach(employee -> {
			...
			muster.get().stream().filter(must -> must.getMusterId().getEmployeeId().equals(employee.getEmployeeId()))
					.collect(Collectors.toList()).forEach(musterData -> {
						if (musterData.getShortHrs() != null) {
							cutlist.setRegularDays(
									cutlist.getRegularDays() + Double.parseDouble(musterData.getShortHrs()));
						}
